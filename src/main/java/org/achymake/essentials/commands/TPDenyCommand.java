...
public class TPDenyCommand extends AbstractPlayerCommand {
    ...
    public TPDenyCommand() {
        super("tpdeny", "denies teleport request", false);
        requirePermission("essentials.command.tpdeny");
    }
    @Override
    protected void execute(@Nonnull CommandContext commandContext,
        ...
        if (getTeleportHandler().isInRequest(playerRef)) {
            var targetRef = getTeleportHandler().getReversed().get(playerRef);
            playerRef.sendMessage(Message.join(
                    Message.raw("You denied ").color(Color.ORANGE),
